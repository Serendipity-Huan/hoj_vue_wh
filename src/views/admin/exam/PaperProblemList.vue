  ...
  <div>
    <el-card>
      ...
      >
        ...
        <vxe-table-column
            min-width="200"
            :title="$t('m.Paper_Problem_Rank')"
        >
          <template v-slot="{ row }">
            <el-input-number
                v-model="paperProblemMap[row.id].rank"
                @change="handleChangeRank(paperProblemMap[row.id])"
                :min="0"
                :max="2147483647"
            ></el-input-number>
          </template>
...
export default {
  ...
  methods: {
    ...
    handleChangeRank(data) {
      api.admin_updatePaperProblem(data).then((res) => {
    ...
    deleteProblem(id) {
      this.$confirm(this.$i18n.t('m.Delete_Problem_Tips'), 'Tips', {
        type: 'warning',
          ...
          () => {
            api
                .admin_deletePaperProblem(id, null)
                .then((res) => {
    ...
    removeProblem(pid) {
      this.$confirm(this.$i18n.t('m.Remove_Paper_Problem_Tips'), 'Tips', {
        type: 'warning',
      }).then(
          () => {
            api
                .admin_deletePaperProblem(pid, this.paperId)
                .then((res) => {
  ...
  watch: {
    $route(newVal, oldVal) {
      if (
          newVal.params.paperId != oldVal.params.paperId ||
