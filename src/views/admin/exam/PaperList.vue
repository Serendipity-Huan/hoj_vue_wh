<template>
  <div>
    <el-card>
      <div slot="header">
        <span class="panel-title home-title">{{ $t('m.Paper_List') }}</span>
      ...
      >
        ...
        <vxe-table-column :title="$t('m.Auth')" width="100">
          <template v-slot="{ row }">
            <el-tag :type="PAPER_TYPE[row.auth]['color']" effect="dark">
              {{ row.auth }}
            </el-tag>
          </template>
        </vxe-table-column>
        <vxe-table-column :title="$t('m.Visible')" min-width="80">
          <template v-slot="{ row }">
            <el-switch
                ...
                @change="changePaperStatus(row.id, row.status, row.author)"
            >
            </el-switch>
          </template>
        </vxe-table-column>
          ...
          <template v-slot="{ row }">
            <template v-if="isSuperAdmin || userInfo.username == row.author">
              <div style="margin-bottom:10px">
                ...
                <el-tooltip
                    effect="dark"
                    :content="$t('m.View_Paper_Problem_List')"
                    placement="top"
                >
                  <el-button
                      icon="el-icon-tickets"
                      size="mini"
                      @click.native="goPaperProblemList(row.id)"
                      type="success"
                  >
                  </el-button>
                </el-tooltip>
            ...
            >
              <el-button
                  icon="el-icon-delete"
                  size="mini"
                  @click.native="deletePaper(row.id)"
                  type="danger"
              >
              </el-button>
            </el-tooltip>
...
import { PAPER_TYPE } from '@/common/constants';
...
export default {
  ...
  mounted() {
    this.getPaperList(this.currentPage);
    this.PAPER_TYPE = Object.assign({}, PAPER_TYPE);
  },
  watch: {
    $route() {
  ...
  methods: {
    ...
    getPaperList(page) {
      this.loading = true;
      api.admin_getPaperList(page, this.pageSize, this.keyword, this.$route.params.examId).then(
          (res) => {
            this.loading = false;
    ...
    deletePaper(paperId) {
      this.$confirm(this.$i18n.t('m.Delete_Paper_Tips'), 'Tips', {
        confirmButtonText: this.$i18n.t('m.OK'),
        cancelButtonText: this.$i18n.t('m.Cancel'),
        type: 'warning',
      }).then(() => {
        api.admin_deletePaper(paperId).then((res) => {
          myMessage.success(this.$i18n.t('m.Delete_successfully'));
          this.currentChange(1);
        });
      });
    },
    changePaperStatus(paperId, status, author) {
      api.admin_changePaperStatus(paperId, status, author).then((res) => {
    ...
    goCreatePaper() {
      this.$router.push({ name: 'admin-create-paper' });
    },
  },
};
</script>
