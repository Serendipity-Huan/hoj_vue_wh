<template>
  <el-card shadow>
    ...
    <div v-show="showTable">
      ...
      >
        ...
        >
          <template v-slot="{ row }">
            <avatar
              :username="row[paper.rankShowName]"
              :inline="true"
              :size="37"
              color="#FFF"
              :src="row.avatar"
              :title="row[paper.rankShowName]"
            ></avatar>
            ...
            <span style="float:right;text-align:right">
              <a @click="getUserHomeByUsername(row.uid, row.username)">
                <span class="paper-username">
                  <span class="paper-rank-flag" v-if="row.uid == userInfo.uid"
                    >Me</span
                  >
                  <span class="paper-rank-flag" v-if="row.rank == -1"
                    >Star</span
                  >
                  <span class="paper-rank-flag" v-if="row.gender == 'female'"
                    >Girl</span
                  >
                  {{ row[paper.rankShowName] }}</span
                >
                <span class="paper-school" v-if="row.school">{{
                  row.school
                }}</span>
              </a>
            </span>
        ...
        <vxe-table-column
          min-width="80"
          v-for="problem in paperProblems"
          :key="problem.displayId"
          :field="problem.displayId"
        >
          <template v-slot:header>
            <span v-if="problem.color" class="paper-rank-balloon">
              <svg
                t="1633685184463"
                class="icon"
            ...
            <span>
              <a
                @click="getPaperProblemById(problem.displayId)"
                class="emphasis"
                style="color:#495060;"
              >
                {{ problem.displayId }}
    ...
    <Pagination
      ...
      :current.sync="page"
      @on-change="getPaperRankData"
      @on-page-size-change="getPaperRankData(1)"
...
import PaperRankMixin from './paperRankMixin';
...
export default {
  ...
  computed: {
    paper() {
      return this.$store.state.paper.paper;
    },
    isMobileView() {
  ...
  methods: {
    ...
    cellClassName({ row, rowIndex, column, columnIndex }) {
      ...
      if (
        ...
        if (this.isPaperAdmin) {
          return row.cellClassName[
            [this.paperProblems[columnIndex - 4].displayId]
          ];
        } else {
          return row.cellClassName[
            [this.paperProblems[columnIndex - 3].displayId]
          ];
        }
      } else {
    ...
    applyToChart(rankData) {
      ...
      for (let i = topIndex; i < len && i < topIndex + 10; i++) {
        let ele = rankData[i];
        user.push(ele[this.paper.rankShowName]);
    ...
    downloadRankCSV() {
      utils.downloadFile(
        `/api/file/download-paper-rank?cid=${
