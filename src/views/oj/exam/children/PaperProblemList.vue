  ...
  <div class="problem-list">
    ...
    >
      ...
      <vxe-table-column
        ...
        title=""
        width="50"
        v-if="isAuthenticated && paperRuleType == RULE_TYPE.OI"
      >
        <template v-slot="{ row }">
      ...
      <vxe-table-column
        ...
        title=""
        width="50"
        v-if="isAuthenticated && paperRuleType == RULE_TYPE.ACM"
      >
        <template v-slot="{ row }">
          <template v-if="isGetStatusOk">
            <el-tooltip
      ...
      <vxe-table-column field="ac" :title="$t('m.AC')" min-width="80">
        <template v-slot="{ row }">
          <span v-if="!PaperRealTimePermission">
            <i
...
export default {
  ...
  mounted() {
    ...
    this.getPaperProblems();
  },
  methods: {
    getPaperProblems() {
      this.$store.dispatch('getPaperProblems').then((res) => {
        if (this.isAuthenticated) {
          let isPaperProblemList = true;
  ...
  computed: {
    ...mapState({
      problems: (state) => state.paper.paperProblems,
    }),
    ...mapGetters([
      'isAuthenticated',
      'paperRuleType',
