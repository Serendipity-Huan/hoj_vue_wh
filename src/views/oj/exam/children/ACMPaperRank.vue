      ...
      >
        ...
        >
          <template v-slot:header>
            <span class="paper-rank-balloon" v-if="problem.color">
              <svg
                t="1633685184463"
                class="icon"
...
import ContestRankMixin from './paperRankMixin';
...
export default {
  ...
  mounted() {
    ...
    this.addChartCategory(this.paperProblems);
  ...
  methods: {
      ...
      if (
        ...
        if (this.isContestAdmin) {
          return row.cellClassName[
            [this.paperProblems[columnIndex - 5].displayId]
          ];
        } else {
          return row.cellClassName[
    ...
    addChartCategory(paperProblems) {
      let category = [];
      for (let i = 0; i <= paperProblems.length; ++i) {
        category.push(i);
      }
    ...
    applyToChart(rankData) {
      ...
      for (let i = topIndex; i < len && i < topIndex + 10; i++) {
        let rank = rankData[i];
        users.push(rank[this.paper.rankShowName]);
        let info = rank.submissionInfo;
        // 提取出已AC题目的时间
        let timeData = [];
        Object.keys(info).forEach((problemID) => {
        ...
        let data = [];
        data.push([this.paper.startTime, 0]);

        for (let [index, value] of timeData.entries()) {
          let realTime = moment(this.paper.startTime)
            .add(value, 'seconds')
            .format();
          data.push([realTime, index + 1]);
        }
        seriesData.push({
          name: rank[this.paper.rankShowName],
          type: 'line',
          data,
        });
      }
