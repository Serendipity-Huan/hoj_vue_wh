      ...
      <el-card shadow>
        <div slot="header">
          <span class="panel-title"
            >{{
              query.type === '' ? $t('m.All') : parseExamType(query.type)
            }}
            {{ $t('m.Exams') }}</span
          >
              ...
              >
                <span class="el-dropdown-link">
                  {{
                    query.status === ''
                      ? $t('m.Status')
                      : $t('m.' + EXAM_STATUS_REVERSE[query.status]['name'])
                  }}
                  <i class="el-icon-caret-bottom"></i>
                </span>
                <el-dropdown-menu slot="dropdown">
        ...
        <div v-loading="loading">
          <p id="no-contest" v-show="exams.length == 0">
          ...
          <ol id="contest-list">
            ...
            >
              <el-row type="flex" justify="space-between" align="middle">
                ...
                >
                  <p class="title">
                    <a class="entry" @click.stop="toExam(exam)">
                      {{ exam.title }}
                    </a>
                    ...
                    <li>
                      <template v-if="exam.type == 0">
                        <el-button
                          ...
                          round
                          :type="'primary'"
                          @click="onRuleChange(exam.type)"
                          ><i class="fa fa-trophy"></i>
                          {{ exam.type | parseExamType }}
                        </el-button>
                      </template>
                      <template v-else>
                        <el-tooltip
                    ...
                    <li v-if="exam.openRank">
                      ...
                      >
                        <el-button
                          ...
                          @click="
                            toExamOutsideScoreBoard(exam.id, exam.type)
                          "
                        ></el-button>
                      </el-tooltip>
                    </li>
...
export default {
  ...
  methods: {
    ...
    getExamList() {
      this.loading = true;
      api.getExamList(this.currentPage, this.limit, this.query).then(
        (res) => {
    ...
    toExamOutsideScoreBoard(eid, type) {
      if (type == 0) {
        this.$router.push({
          name: 'ACMScoreBoard',
          params: { examID: eid },
